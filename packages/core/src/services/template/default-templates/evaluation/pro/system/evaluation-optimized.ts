...
- 评分维度保持不变，但每个维度的给分都必须优先围绕“聚焦点”衡量【优化后消息】在对话上下文中的支撑能力。
- 若优化后消息对聚焦点没有明确、可执行的支持，或与聚焦点存在明显冲突，这是重大缺陷：整体分数不得进入高分区间，并必须在 improvements/patchPlan 中优先修复。

## 快速解释规则（避免误判）

- 若反馈提到“输出/格式/示例/只输出/不要解释/不要赏析/太啰嗦/太长”等，默认指 assistant 最终输出行为/结构；优先调整优化后消息中与输出控制/示例/默认规则相关的内容，而不是盲目删减消息结构或删掉必要的上下文说明。
...
请严格评估优化后消息的效果，并给出进一步改进的通用性建议。{{#hasUserFeedback}}请将用户反馈作为本次评估的核心目标，优先给出围绕该聚焦点的 improvements 与 patchPlan。{{/hasUserFeedback}}`
    }
  ] as MessageTemplate[],
  metadata: {
    version: '3.0.0',
